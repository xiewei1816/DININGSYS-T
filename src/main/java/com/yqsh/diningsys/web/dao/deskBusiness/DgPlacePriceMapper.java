package com.yqsh.diningsys.web.dao.deskBusiness;

import java.util.List;

import org.apache.ibatis.annotations.Param;
import org.springframework.stereotype.Repository;

import com.yqsh.diningsys.core.generic.GenericDao;
import com.yqsh.diningsys.web.model.deskBusiness.DgPlacePrice;
@Repository
public interface DgPlacePriceMapper extends GenericDao<DgPlacePrice,Integer>{
    int deleteByPrimaryKey(Integer id);

    int insert(DgPlacePrice record);

    int insertSelective(DgPlacePrice record);

    DgPlacePrice selectByPrimaryKey(Integer id);

    int updateByPrimaryKeySelective(DgPlacePrice record);

    int updateByPrimaryKey(DgPlacePrice record);
    
    
    List<DgPlacePrice> getAllData(DgPlacePrice record);
    void deleteIds(List<Integer> ids);
    
    int deleteAll();
    
    int insertBackId(DgPlacePrice record);
    
    int deleteByItemId(int id);
    
    int insertChilds(List<DgPlacePrice> record);
    /**
	 * 物理删除所有数据，此方法慎用
	 * @author jianglei
	 * 日期 2017年1月18日 上午9:14:16
	 */
    void delPhy();
	/**
	 * 批量插入
	 * @author jianglei
	 * 日期 2017年1月18日 上午11:07:10
	 * @param listObj
	 */
    void insertBatch(@Param("listObj") List<DgPlacePrice> listObj);
}