$(function(){
	...
	//初始化下拉列表默认选中第一条数据
	$("#outWareId option:first").prop("selected", 'selected');
	$("#inWareId option:first").prop("selected", 'selected');
	...
	$("#" + pageUtil.tabId).getJqGrid({
		url : "DININGSYS/inve/tran/getPageList",
		colM : "serialNumber,outWareName,inWareName,joinItemName,number,unit,dateTime,sinceNumber,remark,createDate",
		queryForm : "queryForm",
		colNames : "流水号,调出仓库,调入仓库,物品名称,数量,物品单位,日期,自编号,备注,创建时间",
		colWid : {"id":40},
	...
	$('#itemTypeId').on('change', function(e, params) {
		$(".leftmenus").lSTreeOption("postData",{wareID:$('#outWareId').val(),itemTypeId:params.selected});
		$(".leftmenus").lSTreeRefresh();
	});
	$('#outWareId').on('change', function(e, params) {
	...
	$("#j-itemTable").on("click",".fa-remove",function(){
		...
		if(isNaN(num)||isNaN(presPrice)||num<=0||presPrice<=0){
			layer.alert('现价与数量请输入正确的数字',{title :'提示信息',icon: 0, skin: 'layer-ext-moon'});
			return false;
		}
		var inveNum = $(this).closest("tr").find("td[aria-describedby$='_inveNum']").attr("title");
		if(parseFloat(num>inveNum)){
			layer.alert('库存数量不足',{title :'提示信息',icon: 0, skin: 'layer-ext-moon'});
			return false;
		}
	...
	$('.j-tran').click(function(){
		...
		layer.open({
			  ...
			  yes: function(index, layero){
				  var outWareId=$('#outWareId').val();
					 var inWareId=$('#inWareId').val();
					 if(outWareId===inWareId){
						 layer.alert('调出仓库与调入仓库不能相同！',{title :'提示信息',icon: 0, skin: 'layer-ext-moon'});
						 return false;
					 }
				    //获取是否有值处于编辑状态
				 ...
				 $.post('DININGSYS/inve/tran/saveTransfers',$('#editForm').serialize(),function(data){
	...
	$("#j-itemTable").jqGrid({
        datatype : "json",
        colNames : ['编号','物品编码','物品名称','原价','现价','库存数量','调拨数量','单位','自编号','金额','备注','操作'],
        colModel : [ 
	...
	initTree($('#outWareId').val(),$('#itemTypeId').val());//初始化
});
