$(function () {
    ...
    $.ajax({
        type:'POST',
        url:path+'/DININGSYS/mealTime/getTableItem',
        dataType:'JSON',
        success:function (data) {
            if(data.success){
            	 for (var i in data.items) { 
            		 ...
            		 colNames.push(data.items[i].bisName); //赋值名称title
                     colModelData.push(colData);
            	 }
            	 
                 //动态增加列
                 $("#grid-table").jqGrid({
                     url:"DININGSYS/mealTime/getAllData",
    ...
    $("#add").click(function(){
    	$.get(path+"/DININGSYS/mealTime/toAddPlanDish",function(str){
            var addsIndex = layer.open({
                ...
                yes:function () {
	                  ...
	                  $.ajax({
	                  type:'POST',
	                  url:path+'/DININGSYS/mealTime/getMealTimeAddDish',
   ...
   $("#refresh").click(function(){
    ...
    $("#save").click(function(){
        layer.confirm('确认保存当前市别品项价格?', {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                    type:'POST',
                    url:path+'/DININGSYS/mealTime/saveData',
                    data:{data:send},
                    dataType:'JSON',
                    success:function (data) {
                        if(data.success){
    ...
    $("#delb").click(function(){
        ...
        $.ajax({
                type:'POST',
                url:'DININGSYS/mealTime/delData',
