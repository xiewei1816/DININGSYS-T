$(function () {
	...
	$("#grid-table").jqGrid({
        url:"DININGSYS/NewestItem/getAllData",
    ...
    $("#add").click(function(){
    	$.get("DININGSYS/NewestItem/toAddItem",function(str){
            var addsIndex = layer.open({
                ...
                yes:function () {
                      ...
                      $.ajax({
                      type:'POST',
                      url:'DININGSYS/NewestItem/getPanItemByAdd',
    ...
    $("#save").click(function(){
		...
		var dish_content = "";
  		...
  		layer.confirm('确认保存当前选择的品项数据?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                    type:'POST',
                    url:path+'/DININGSYS/NewestItem/saveData',
    ...
    $("#export").click(function(){
		window.location.href=path+"/DININGSYS/NewestItem/exportXls";
	});
    $("#delb").click(function(){
		var grid = $("#grid-table");
        ...
        $.ajax({
                type:'POST',
                url:'DININGSYS/NewestItem/delData',
