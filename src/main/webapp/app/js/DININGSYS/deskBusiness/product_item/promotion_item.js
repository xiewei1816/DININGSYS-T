$(function () {
	...
	pageUtil.pageInit({
     ...
     var mCount = $(".m-count");
    $("#grid-table").jqGrid({
        url:"DININGSYS/DgPromotion/getAllData",
        ...
        width:600,
        rowNum:-1,
        colNames:["id","品项id","品项编号","品项名称","标准价格","促销价格","每客最大点单数量","促销价格","每客最大点单数量"],
        colModel:[
            ...
            {name:'proPriceText',index:'proPriceText', width:'10%',align:'center',formatter:format1},
            {name:'maxCountText',index:'maxCountText', width:'20%',align:'center',formatter:format2},
            {name:'proPrice',index:'proPrice', width:'0%',align:'center',hidden:true},
            {name:'maxCount',index:'maxCount', width:'0%',align:'center',hidden:true}
        ],
    ...
    $("#add").click(function(){
    	$.get(path+"/DININGSYS/DgPromotion/toAddPlanDish",function(str){
            var addsIndex = layer.open({
                ...
                yes:function () {
	                  ...
	                  $.ajax({
	                  type:'POST',
	                  url:path+'/DININGSYS/DgPromotion/getMealTimeAddDish',
   ...
   $("#refresh").click(function(){
    ...
    $("#save").click(function(){
        ...
        layer.confirm('确认保存当前品项价格?', {icon: 3, title:'提示'}, function(index){
        	var send='[';
        	var hava_rows = $("#grid-table").jqGrid('getRowData');
        	...
        	if(hava_rows.length > 0){
                for (var i in hava_rows) {
                	...
                	var price=hava_rows[i].sPrice;
                	if(parseFloat(proPrice)>parseFloat(price)){
                		layer.alert('【'+itemCode+'】促销价格不能大于标准价格',{title :'提示',icon:2, skin: 'layer-ext-moon'});
                		return false;
                	}
            ...
            $.ajax({
                    type:'POST',
                    url:path+'/DININGSYS/DgPromotion/saveData',
                    data:{data:send},
                    dataType:'JSON',
                    success:function (data) {
                        if(data.success){
    ...
    $("#delb").click(function(){
        ...
        $.ajax({
                type:'POST',
                url:'DININGSYS/DgPromotion/delData',
...
function format1(c,o,r){
	...
	{
		return "<input class='item-price' type='text'  id='price-"+r.id+"' style='width:70px;' value='"+r.proPrice+"' dete='em-money'/>";	
	}
}
function format2(c,o,r){
	return "<input class='item_count' type='text'  id='count-"+r.id+"' style='width:100px;' value='"+r.maxCount+"' dete='em-money'/>";
}