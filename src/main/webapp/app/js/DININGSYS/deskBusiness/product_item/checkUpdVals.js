function checkUpdVals(form){
		var messages = true;
		$("#"+form+" .isnumber").each(function(i,e){
			var val = $(e).val();
			var label = $(e).closest(".form-group").find(".title").text();
			if(isNaN(Number(val))){
		...
		$("#"+form+" [dete]").each(function(i,e){
			...
			var label = $(e).closest(".form-group").find("."+$(e).attr("name")+"_title").text().replace("*","");
			switch(date){
				case "val" : {
					if($(e).val() == ""){
                    ...
                    break;
				case "clstype" : {
					if($(e).val() == "" || $(e).val() == '0' || $(e).val() == null){ //为空或者为0
