var print_dish = function () {
	function pagerInit()
    {
		...
		$("#grid-table-4").jqGrid({
	        url:"DININGSYS/PrintManger/getAllItem",
	        datatype:"json", //数据来源，本地数据
	        mtype:"POST",//提交方式
	        height:150,//高度，表格高度。可为数值、百分比或'auto'
	        width:630,//这个宽度不能为百分比
	        rowNum:-1,
	        colNames:["id","编号","名称","单位","标准价格","说明"],
	        colModel:[
		...
		$("#grid-table-5").jqGrid({
	        url:"DININGSYS/PrintManger/getHaveItem",
	        datatype:"json", //数据来源，本地数据
	        mtype:"POST",//提交方式
	        height:150,//高度，表格高度。可为数值、百分比或'auto'
	        width:630,//这个宽度不能为百分比
	...
	var discounts_setting = {
		    ...
		    enable : true,//是否异步加载
	        url : 'DININGSYS/ProDiscount/getTreeNodes',//加载数据的url  /DININGSYS/ProDiscount/getTreeNodes
