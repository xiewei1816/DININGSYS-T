var dish_add = function () {
	var treeNodeId='';
	function pagerInit(grid,yxdz,disable)
    {
		var allUrl;
		var allowUrl;
		if(yxdz != undefined  && yxdz != null && yxdz != '')
		{
			if(disable != undefined  && disable != null && disable != '')
			{
				allUrl = path+"/DININGSYS/ProDiscountPan/getAllDish?yxdz=0&disable=0";
				allowUrl = path+"/DININGSYS/ProDiscountPan/getHaveDish?yxdz=0&disable=0";
			}
			else
			{
				allUrl = path+"/DININGSYS/ProDiscountPan/getAllDish?yxdz=0";
				allowUrl = path+"/DININGSYS/ProDiscountPan/getHaveDish?yxdz=0";
			}
		}
		else
		{
			allUrl = path+"/DININGSYS/ProDiscountPan/getAllDish";
			allowUrl = path+"/DININGSYS/ProDiscountPan/getHaveDish";
		}
		...
		$("#grid-table-4").jqGrid({
	        ...
	        colModel:[
	            ...
	            {name:'standardPrice',index:'standardPrice', width:'10%',align:'center'},
	            {name:'costPrice',index:'costPrice', width:'10%',align:'center'},
	            {name:'pxxtsm',index:'pxxtsm', width:'10%',align:'center'}
	        ],
	...
	return {
	        ztreeInit:function () {
	 		...
	 		pagerInit:function (grid,yxdz,disable){
	 			pagerInit(grid,yxdz,disable);
	 		}
	}
}();
